...
The `Lint and Test` workflow serves both `pull_request` and trusted `push` events. Its original SonarCloud job was restricted to a push on `main`, received `SONAR_TOKEN`, and invoked the repository's container-backed `sonar:scanner:cli` task. Trusted runs reached the scanner but failed because the image's non-root UID could not create `.scannerwork` in the bind-mounted GitHub workspace. Pull-request runs skipped the job to avoid exposing the secret. After the official scanner action removed the workspace blocker, live run `29914871551` reached SonarQube Cloud but failed to load settings for `Ensono_eirctl`, reported `NOT_FOUND` and a `NONEXISTENT` binding, and ended with “Not authorized or project not found.” Organization `ensono` and project key `Ensono_eirctl` remained fixed while authorized access was obtained, the canonical binding and `main` branch were verified, the Base plan was confirmed, and a maintained-identity analysis credential replaced the invalid repository secret without exposing either value.

Subsequent live pull-request analysis exposed two independent path defects. GitHub's artifact action downloaded the selected reports at the artifact root, and then repository-relative Go coverage keys failed to match files isolated beneath `analysis/source`. Protected normalization now accepts the exact bounded root-level artifact, prefixes canonical coverage keys with `source/`, and retains the scanner's fixed `reports/.coverage/*` paths. Exact-artifact replay proves 40,768 records across 55 unique coverage paths match the API-materialized Go source with zero missing files. PR #126 and #129 quality-gate failures occurred before the namespace correction reached protected `main` and are not post-fix acceptance evidence. After merge, trusted-main run `30004395523` loaded `.coverage/out` successfully and submitted analysis, but the gate failed on **Critical Code Smells Severity on New Code**. Authenticated Sonar data identifies 19 open Critical smells in the previous-version period since `0.11.3`; current-line Git blame attributes 12 to this change's PR #114, four to earlier PR #102, two to unrelated PR #99, and one to legacy PR #51. The remaining design problem is therefore fresh live PR acceptance plus remediation of the 12 change-attributable findings, while the seven earlier findings remain openly documented debt rather than being suppressed or reclassified.
...
- Preserve Go coverage and JUnit report ingestion, and distinguish successful report import from the independent final quality-gate result.
...
GitHub may return an empty `pull_requests` array for a valid fork-originated `pull_request` Actions run. Protected provenance resolution first validates the run's base repository, event, full head SHA, head repository, and non-empty head branch. When the run contains exactly one pull-request association, it resolves that number through the base repository API as before. When the association array is empty, protected code queries open pull requests in `Ensono/eirctl` using the verified fork owner and exact head branch plus base branch `main`, requires exactly one result, and then requires that result's base repository, base ref, head repository, head ref, and current head SHA to exactly match the already verified run. A missing owner or branch, more than one run association, zero or multiple lookup results, a closed candidate, or any repository/ref/SHA mismatch fails closed before artifact download or secret exposure. The fallback never trusts a pull-request number, repository, ref, or SHA supplied by fork content.

Per-PR/revision concurrency cancels stale analysis. When the event payload contains a pull-request number, the concurrency key uses it; otherwise the key uses the verified workflow-run head repository identity and head branch so unrelated fork runs cannot share the empty `sonar-pr-` group while newer revisions of the same fork branch still cancel stale analysis. The source materializer performs a final current-head check before it exits so a revision changed during API retrieval fails closed. A later race is handled by concurrency cancellation; artifacts and source identities are never mixed across revisions.
...
Organization `ensono` and project key `Ensono_eirctl` are fixed inputs and are not migration choices. An authorized `ensono` administrator has confirmed through authenticated SonarQube Cloud access that the existing canonical project is bound to `Ensono/eirctl` and uses `main` as its main branch. The public `Ensono_taskctl` project remains historical public evidence only; it is not a substitute key or an assumed migration source. No implementation step renames the fixed project key, selects an alternate key, or creates a blind duplicate.
...
The repository secret remains named `SONAR_TOKEN`. The confirmed Base plan does not provide the project-scoped Scoped Organization Token available on higher plans, so a personal access token from a maintained identity with only the authorization required to analyze the canonical project replaced the invalid value. The credential owner and expiry are recorded in the team's secret-management process rather than in the repository. No token value is retrieved, copied into diagnostics, or replaced through an unauthorized workaround.
...
- **[A fork run omits pull-request associations]** → Resolve only open pull requests through a protected base-repository query scoped to the verified fork owner, exact head branch, and `main`; require exactly one candidate and revalidate its base repository/ref, head repository/ref, and immutable SHA. Reject absent, ambiguous, stale, closed, or mismatched results before artifact download or secret exposure.
...
- **[A failing quality gate is misdiagnosed as missing coverage or wholly attributed to this change]** → Treat scanner report discovery, path resolution, analysis submission, and the final gate as separate evidence. When the Go Cover sensor loads the verified report with no unresolved paths, retain the established report and `source/` mapping. Use authenticated issue metadata plus current-line Git blame to remediate every Critical smell introduced by PR #114, and record findings blamed to PRs #51, #99, and #102 as open pre-existing debt without suppression, acceptance-state changes, threshold reduction, or ruleset bypass.
...
8. Preserve organization `ensono` and project key `Ensono_eirctl` as fixed while authorized administration confirms the canonical binding, Base plan, and main branch without creating a duplicate.
9. Replace the invalid repository secret with the plan-supported maintained-identity credential, record its owner and expiry outside the repository, prove trusted-main project-settings and analysis submission, and revoke the superseded value.
10. Retain the completed root-level artifact and `source/` coverage-namespace corrections on protected `main`; do not treat earlier PR #126 or #129 checks as validation because their analyzers executed before the namespace correction merged.
11. Trigger a fresh same-repository PR run using current protected analyzer code. Verify exact revision, zero unresolved coverage paths, non-zero coverage import, PR decoration, new-code behavior, token isolation, and no source execution.
12. Use authenticated SonarQube Cloud issue details and current-line Git blame to classify the broad previous-version quality-gate debt. Remediate all 12 Critical smells attributed to PR #114 without changing the verified coverage mapping, suppressing or reclassifying findings, reducing the gate, or bypassing the ruleset; rerun trusted main to prove coverage still loads and no Critical smell attributable to this change remains. Record the seven findings blamed to PRs #51, #99, and #102 as open pre-existing debt even if they keep the broad main quality gate red.
13. Exercise the adversarial fork PR. If GitHub omits the run's pull-request association, add the protected exact-match fallback and fork-safe concurrency key, rerun policy and hostile provenance tests, then verify exact revision, coverage, decoration, new-code behavior, token isolation, and no source execution.
14. Confirm code-owner enforcement and add the observed external SonarCloud check identity to `main-is-main`.
15. Record final workflow URLs, canonical project and binding state, credential type/owner/expiry without its value, ruleset state, release/SHA evidence, API bounds, coverage-import evidence, quality-gate remediation, and residual parser risk.
...
- Confirm through authenticated SonarQube Cloud issue details after remediation that no open Critical smell in the previous-version new-code period remains attributable by current-line Git blame to PR #114; retain the seven earlier findings as explicitly documented open debt.
- Confirm through a fresh post-namespace-fix same-repository PR and an adversarial fork PR that an allowlisted source tree without `.git` provides non-zero coverage import, acceptable SonarCloud PR decoration, and correct new-code behavior.
- Confirm the exact SonarCloud external quality-gate context and integration ID from a successful live PR analysis before changing required checks.
