...
Protected PR analysis run `30104143411` analyzed exact PR #132 revision `31c31f86e6bd7513b88a1d92e1686c09a5726cdd`. External check `SonarCloud Code Analysis` (`89517665288`) passed with zero new issues, zero open Critical code smells, **86.3% Coverage on New Code**, zero security hotspots, and 0.0% duplication.

PR #132 merged to protected `main` as `bb3d10dc2360a50752e2c73fe507c427bf14af92` on `2026-07-27T08:31:08Z`. Trusted-main [`Lint and Test` run 30250212009](https://github.com/Ensono/eirctl/actions/runs/30250212009) generated the reports and loaded `/home/runner/work/eirctl/eirctl/.coverage/out` in the Go Cover sensor before submitting analysis for that exact merge commit. The immutable scanner completed analysis but correctly failed quality-gate waiting because the seven documented pre-existing Critical smells still make the broad previous-version condition `CRITICAL Code Smells Severity on New Code` exceed its required `< MAJOR` threshold; external check [`SonarCloud Code Analysis` 89927788808](https://github.com/Ensono/eirctl/runs/89927788808) records that condition. Combined with the exact-revision PR result showing zero open Critical smells after the 12 refactors, this post-merge trusted-main run proves the verified coverage path remains intact and no Critical smell attributable to PR #114 remains. The seven issues blamed to PRs #51, #99, and #102 remain open, unsuppressed, and unreclassified as explicit pre-existing debt.

## Final Operational Record — 2026-07-27

- **Workflow evidence:** trusted-main run [`30250212009`](https://github.com/Ensono/eirctl/actions/runs/30250212009) analyzed merge commit `bb3d10dc2360a50752e2c73fe507c427bf14af92`; same-repository protected PR run [`30087572540`](https://github.com/Ensono/eirctl/actions/runs/30087572540) analyzed exact PR #126 revision `250cd0d5dd81bc5c02f7cb556ca0f8a8685dcd0c`; synchronized adversarial-fork run [`30091307667`](https://github.com/Ensono/eirctl/actions/runs/30091307667) analyzed exact PR #130 revision `ce9e504afaac8d892bc5e57bf9597a4760627176`; and remediation run [`30104143411`](https://github.com/Ensono/eirctl/actions/runs/30104143411) analyzed PR #132 with zero open Critical smells. The stable external context is `SonarCloud Code Analysis` from SonarQubeCloud integration ID `12526`.
- **Canonical project and credential:** authenticated administration confirms organization `ensono`, project key `Ensono_eirctl`, binding to `Ensono/eirctl`, main branch `main`, and the Base plan. The repository secret contains a minimally authorized maintained-identity personal access token because the plan does not provide project-scoped Scoped Organization Tokens. Its owner and expiry are recorded in the team's secret-management process; no credential value or private owner/expiry record is duplicated here. The repository secret was updated at `2026-07-23T07:53:41Z`, the replacement succeeded, and the superseded credential was revoked.
- **Ruleset state:** active `main-is-main` ruleset `5842755` requires one approval, matching code-owner review, last-push approval, resolved review threads, CodeQL `high_or_higher`, and exact checks `Lint`, `Test (Linux)`, and `SonarCloud Code Analysis`; it has no bypass actors. `@Ensono/digital-tools-maintainers` owns workflows, `CODEOWNERS`, and `sonar-project.properties`.
- **Immutable runtime evidence:** implementation-time latest stable selections are `actions/checkout@v7.0.1` (`3d3c42e5aac5ba805825da76410c181273ba90b1`), `actions/setup-go@v7.0.0` (`b7ad1dad31e06c5925ef5d2fc7ad053ef454303e`), `actions/upload-artifact@v7.0.1` (`043fb46d1a93c77aae656e7c1c64a875d1fc6a0a`), `actions/download-artifact@v8.0.1` (`3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c`), and `SonarSource/sonarqube-scan-action@v8.2.1` (`22918119ff8e1ca75a623e15c8296b6ea4fbe28f`). The approved action installs signature-verified SonarScanner CLI `8.1.0.6389`; immutable-dependency and selected-action checks pass.
- **Passive-input bounds and coverage:** protected limits remain 384 recursive tree entries, 160 regular Go files, 160-byte paths, 128 KiB per Go blob, and 1 MiB aggregate Go source. Reports remain exactly root-level `out` and `report-junit.xml` before protected normalization to `reports/.coverage/`; repository-relative coverage keys remain normalized only by the fixed `source/` prefix. Exact-artifact replay matched all 40,768 records across 55 unique coverage paths with zero missing materialized files. Same-repository and fork checks reported 86.2% and 86.4% coverage on new code respectively, while the final trusted-main run again loaded `.coverage/out`.
- **Critical-smell disposition:** all 12 findings blamed to PR #114 were remediated through focused refactoring and are absent from the exact-revision PR analysis after merge. The seven authenticated findings blamed to PRs #51, #99, and #102 remain open as pre-existing debt with their issue keys, rules, locations, and messages recorded in the attribution table above. No issue was suppressed, accepted, reclassified, or dismissed, and neither the quality gate nor the ruleset was reduced or bypassed.
- **Residual risk:** the pinned scanner still parses attacker-controlled, bounded Go source and reports while holding the analysis token. The retained mitigation is an ephemeral runner with no pull-request checkout, Git metadata, cache, dependency, script, workflow, local action, container, binary, or untrusted scanner configuration; only non-executable API-verified Go blobs and bounded reports reach the scanner, trusted settings are forced, and `SONAR_TOKEN` exists only on the scanner step.
- **Rollback:** first remove only the exact `SonarCloud Code Analysis` integration `12526` required check from `main-is-main`, then disable the trusted pull-request analyzer. Preserve CODEOWNERS, the no-privileged-checkout policy, lint/test/CodeQL requirements, canonical project identity, and credential controls. Do not restore the rejected checkout design, invalid credential, alternate project key, weaker threshold, or ruleset bypass.
