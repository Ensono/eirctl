...
- [x] 5.11 Handle fork-originated Actions runs whose `pull_requests` array is empty by resolving exactly one open PR through the protected base-repository API using the verified fork owner, head branch, and `main`; revalidate the candidate repository, refs, and current full SHA; add a fork-safe concurrency fallback; and add hostile workflow-policy tests for absent, ambiguous, stale, closed, and mismatched candidates.
...
**Current dependencies:** Authorized access, canonical binding, credential rotation, the root-level artifact contract, and `source/` coverage-namespace normalization are complete and present on protected `main`. Fresh same-repository PR #126 run `30087572540` and synchronized adversarial fork PR #130 run `30091307667` prove the corrected namespace, non-zero coverage, exact-revision PR decoration, passing quality gates, acceptable no-`.git` behavior, the passive-only boundary, and static-policy/CodeQL acceptance for both repository origins. Trusted-main runs `30004395523` and `30089934950` independently prove that `.coverage/out` loads, then fail on Critical Code Smells Severity in the broad post-`0.11.3` period. Authenticated issue data and current-line Git blame attribute 12 Critical smells to this change's PR #114 and seven to earlier PRs #51, #99, and #102. Task 8.10 must remediate the 12 in-scope findings without changing verified coverage mapping or weakening policy; task 8.12 records the seven still-open pre-existing findings and final state.
...
- [x] 8.6 Synchronize PR #126 with current `main` or trigger an equivalent fresh same-repository pull-request run after the namespace correction is protected; confirm the exact current head SHA, non-zero imported Go coverage, zero unresolved coverage paths, SonarCloud PR decoration, quality-gate result, acceptable no-`.git` new-code behavior, and no secret exposure in the untrusted workflow. Do not accept the stale pre-merge PR #126 or #129 Sonar results as evidence.
- [x] 8.7 Exercise a fork-originated PR with adversarial Git tree entries, `sonar-project.properties`, scripts, workflows, local actions, dependency hooks, container definitions, and inert executable-looking files; confirm forbidden entries fail closed or remain unmaterialized, trusted settings win, no fork content executes, and SonarCloud decorates the exact fork revision.
- [x] 8.8 Use the same-repository and fork exercises to confirm and document the API-materialized source tree's SonarCloud SCM/new-code fidelity and passive-only boundary without `.git`, then rerun static policy and CodeQL validation for the selected form.
...
- [x] 8.10 Use authenticated SonarQube Cloud issue details and current-line Git blame to classify the 19 open Critical smells in the post-`0.11.3` period; remediate all 12 findings attributed to this change's PR #114 without changing the verified `.coverage/out` or `source/` mapping, suppressing or reclassifying issues, reducing the quality gate, or bypassing the ruleset; rerun trusted main and confirm coverage loads and no Critical smell attributable to PR #114 remains, while documenting the seven findings blamed to PRs #51, #99, and #102 as open pre-existing debt.
- [x] 8.11 Observe the stable external SonarCloud check context and integration ID from a successful fresh live PR analysis, add that exact check to `main-is-main`, and verify a missing or failing quality gate blocks merge while a passing gate satisfies the rule.
- [x] 8.12 Record final workflow URLs, canonical project and binding state, credential type/owner/expiry without its value, ruleset state, action and scanner release/SHA/version evidence, source bounds, coverage-import evidence, remediation of the 12 PR #114 Critical smells, the seven still-open pre-existing findings with blame evidence, residual scanner-parser risk, and rollback information in the change verification notes.
