...
- Correct publisher selection: https://github.com/Ensono-Staging/eirctl-test/actions/runs/32253650040. `validate-build` authenticated the run, artifact, current PR head, and provenance; `publish` stopped in `waiting` at protected environment approval. The run was cancelled and no release was created.
...
- Cleanup: both test artifacts were deleted; the correct publisher run was cancelled; no release existed; the test PR was closed and its branch deleted; the artificial stale-head delay was reverted; Actions were re-enabled. Staging `main` retains only the fixed SemVer adaptation required by the organization action allowlist.
