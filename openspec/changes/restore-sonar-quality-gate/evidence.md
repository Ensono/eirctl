...
- `AZWKA6j703lIpGBL3rEk` (Tier 2): **blocked pending scope review.** Task 2.4 specifies five repeats of the `no-prompt` external key, but the current `cmd/eirctl/init.go` contains four: two Viper lookups, the flag declaration, and the Viper binding/lookup. All four use the same external flag/key, but the stated precondition does not match the implementation shape. No source behavior was changed and the task remains incomplete.
