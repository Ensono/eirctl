...
- Treat organization `ensono` and project key `Ensono_eirctl` as fixed. After authorized access is granted, verify the existing project's binding to `Ensono/eirctl` and main branch `main`, then replace the repository `SONAR_TOKEN` value with a current credential supported by the organization plan. Do not rename the project key, substitute an alternate key, or create a blind duplicate. Prefer a project-scoped Scoped Organization Token where available because it grants only **Execute analysis**; otherwise use a personal access token with the minimum analysis authorization required by the Free plan.
...
- Affected Sonar configuration and administration: `sonar-project.properties`, trusted scanner arguments/configuration, coverage and JUnit artifact handling, authenticated validation of the fixed project's repository binding and main branch, and rotation plus authorization validation of the `SONAR_TOKEN` repository secret after access is granted.
...
- Operations: while access is pending, preserve the failed-run, public-metadata, secret-metadata, and CodeQL evidence and make no project-identity or credential changes. After access is granted, first verify that authenticated project settings load for the fixed, bound `Ensono_eirctl` project with the rotated credential; then run one trusted `main` push, one live same-repository PR, and one live fork PR to verify analysis association, token isolation, quality-gate reporting, and ruleset enforcement.
