...
- **[A failing quality gate is misdiagnosed as missing coverage]** → Treat scanner report discovery, path resolution, analysis submission, and the final gate as separate evidence. When the Go Cover sensor loads the verified report with no unresolved paths, retain the established report and `source/` mapping; identify and remediate the reported quality issue without suppression, threshold reduction, or ruleset bypass.
...
12. Use authenticated SonarQube Cloud issue details to identify the exact rule, file, and line behind trusted-main run `30004395523`'s Critical Code Smell. Remediate the issue without changing the verified coverage mapping, suppressing the finding, reducing the gate, or bypassing the ruleset; rerun trusted main to a passing gate.
...
- Identify through authenticated SonarQube Cloud issue details the exact rule, file, and line responsible for trusted-main run `30004395523`'s Critical Code Smell before selecting a remediation.
