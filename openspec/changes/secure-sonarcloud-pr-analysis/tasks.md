...
**Current dependencies:** Authorized access, canonical binding, credential rotation, the root-level artifact contract, and `source/` coverage-namespace normalization are complete and present on protected `main`. Fresh same-repository PR #126 run `30087572540` proves the corrected namespace, non-zero coverage, PR decoration, and passing quality gate. Adversarial fork PR #130 run `30088667451` then proved that GitHub can omit fork PR associations from the upstream Actions run, causing the current analyzer to fail closed during provenance resolution; task 5.11 must correct and protect that lookup before task 8.7 is rerun. Trusted-main run `30004395523` independently proved that `.coverage/out` loads, then failed on a Critical Code Smell; task 8.10 must resolve that genuine gate condition without changing the verified coverage mapping or weakening policy. Tasks 8.7–8.8 and 8.11–8.12 remain sequenced after the required live acceptance; independently completed governance task 8.9 is not blocked.
...
- [ ] 8.11 Observe the stable external SonarCloud check context and integration ID from a successful fresh live PR analysis, add that exact check to `main-is-main`, and verify a missing or failing quality gate blocks merge while a passing gate satisfies the rule.
