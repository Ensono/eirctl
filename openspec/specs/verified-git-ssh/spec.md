...
The system SHALL resolve user and system known-host files using OpenSSH-compatible configuration, including the effective hostname and port after host aliases and supported `GIT_SSH_COMMAND` options are applied.
...
- **THEN** the system uses readable standard user and system known-host locations
