...
grep -Eq "^[[:space:]]+name: mirror\.gcr\.io/golang:${version}-trixie(@sha256:[0-9a-f]{64})?$" shared/build/go/eirctl.yaml || fail "shared Go build context is not golang:${version}-trixie"
